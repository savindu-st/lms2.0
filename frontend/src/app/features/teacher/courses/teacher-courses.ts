...
@Component({
    ...
    <div class="container teacher-page">
      ...
      <div class="analytics-grid">
        <div class="card stat-card">
          ...
          <div class="stat-val mono-num text-primary">\${{ analytics()?.totalRevenue | number:'1.2-2' }}</div>
        ...
        <div class="card stat-card alert-clickable" routerLink="/teacher/gradebook">
          ...
          <div class="stat-val mono-num text-primary">{{ analytics()?.pendingSubmissionsCount }}</div>
      ...
      <div class="dash-management-header">
        <div>
          ...
          <p class="text-secondary">Configure monthly cohorts, curriculum modules, lessons, and access validity periods.</p>
        </div>

        <button (click)="openCreateCourseModal()" class="btn btn-primary btn-sm">
          ...
          <table class="table">
            ...
            <tbody>
              @for (c of courses(); track c.id) {
                <tr>
                  <td>
                    <span class="badge badge-zinc">{{ c.monthYear }}</span>
                    <div class="mono-num text-muted" style="font-size: 0.6875rem; margin-top: 2px;">{{ c.courseCode }}</div>
                  </td>
                  <td>
                    <strong>{{ c.title }}</strong>
                  </td>
                  <td class="mono-num font-bold text-primary">\${{ c.price | number:'1.2-2' }}</td>
                  <td class="mono-num" style="font-size: 0.8125rem;">{{ c.accessDurationDays }} Days</td>
                  <td>
                    <span class="text-secondary" style="font-size: 0.8125rem;">{{ c.modulesCount }} Mods &bull; {{ c.lessonsCount }} Lessons</span>
                  </td>
                  <td class="mono-num" style="font-size: 0.8125rem;">{{ c.activeStudentsCount }} Enrolled</td>
                  <td>
                    <span class="badge" [class.badge-emerald]="c.isPublished" [class.badge-zinc]="!c.isPublished">
                      {{ c.isPublished ? 'Published' : 'Draft' }}
                    </span>
                  </td>
                  <td>
                    <div class="action-btns">
                      <button (click)="openCurriculumBuilder(c)" class="btn btn-outline btn-sm" title="Manage Modules & Lessons">
                        Curriculum
                      </button>
      ...
      @if (showCourseModal()) {
        <div class="modal-overlay" (click)="onModalBackdrop($event)">
          <div class="modal-content">
            <button class="modal-close" (click)="showCourseModal.set(false)" title="Close" aria-label="Close">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
            </button>

            <h3>{{ isEditing() ? 'Update Monthly Class' : 'Create New Monthly Class' }}</h3>
            <p class="text-secondary" style="margin-bottom: 1.25rem; font-size: 0.8125rem;">Configure cohort details, tuition fee, and access duration.</p>

            <form (ngSubmit)="saveCourse()">
              <div class="form-group">
                <label class="form-label">Course Title *</label>
                <input type="text" class="form-control" [(ngModel)]="courseForm.title" name="cTitle" placeholder="e.g. Advanced Auditing & Risk Modeling" required />
              </div>
              ...
              <div class="form-group">
                ...
                <textarea class="form-control" [(ngModel)]="courseForm.description" name="cDesc" rows="3" placeholder="Outline accounting competencies, learning goals, and practical exercises..."></textarea>
              </div>

              <div class="form-group" style="display: flex; flex-direction: row; align-items: center; gap: 0.5rem; margin-top: 0.25rem;">
                <input type="checkbox" id="pubCheck" [(ngModel)]="courseForm.isPublished" name="cPub" />
                <label for="pubCheck" class="form-label" style="margin-bottom: 0; cursor: pointer;">Publish immediately to Course Catalog</label>
              </div>

              <div class="modal-footer" style="margin-top: 1.25rem; display: flex; justify-content: flex-end; gap: 0.5rem;">
                <button type="button" (click)="showCourseModal.set(false)" class="btn btn-outline btn-sm">Cancel</button>
                <button type="submit" [disabled]="savingCourse()" class="btn btn-primary btn-sm">
      ...
      @if (activeCurriculumCourse()) {
        <div class="modal-overlay" (click)="onCurriculumBackdrop($event)">
          <div class="modal-content modal-content-lg curriculum-builder-modal">
            <button class="modal-close" (click)="activeCurriculumCourse.set(null)" title="Close" aria-label="Close">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
            </button>

            <div class="builder-header">
              <div class="badge badge-zinc" style="margin-bottom: 0.35rem;">{{ activeCurriculumCourse()?.monthYear }} Cohort</div>
              <h2>Curriculum: {{ activeCurriculumCourse()?.title }}</h2>
              <p class="text-secondary">Structure modules, video streaming lectures, reading materials, and auto-graded quizzes.</p>
            </div>
            ...
            <div class="add-module-bar">
              <input
                ...
                placeholder="New module title (e.g. Module 3: Cash Flow Statements)..." />
              <button (click)="addModule()" [disabled]="!newModuleTitle" class="btn btn-primary btn-sm">
                + Add Module
              </button>
            </div>
                ...
                <div class="builder-module-card card">
                  ...
                  <div class="builder-lessons">
                    @if (mod.lessons.length === 0) {
                      <p class="text-muted" style="font-size: 0.8125rem; padding: 0.35rem 0;">No lessons in this module yet. Click "+ Add Lesson".</p>
                    }
                    @for (les of mod.lessons; track les.id) {
                      <div class="builder-lesson-row">
                        <div class="b-les-info">
                          <span class="badge badge-zinc" [ngSwitch]="les.contentType">
                          ...
                          <span class="text-muted mono-num" *ngIf="les.durationMinutes" style="font-size: 0.75rem;">({{ les.durationMinutes }}m)</span>
                        </div>
      ...
      @if (showLessonModal()) {
        <div class="modal-overlay" (click)="showLessonModal.set(false)">
          <div class="modal-content" (click)="$event.stopPropagation()">
            <button class="modal-close" (click)="showLessonModal.set(false)" title="Close" aria-label="Close">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
            </button>
            ...
            <form (ngSubmit)="saveLesson()">
              ...
              <div class="modal-footer" style="display: flex; justify-content: flex-end; gap: 0.5rem; margin-top: 1rem;">
                <button type="button" (click)="showLessonModal.set(false)" class="btn btn-outline btn-sm">Cancel</button>
      ...
      @if (showQuizModal()) {
        <div class="modal-overlay" (click)="showQuizModal.set(false)">
          <div class="modal-content modal-content-lg" (click)="$event.stopPropagation()">
            <button class="modal-close" (click)="showQuizModal.set(false)" title="Close" aria-label="Close">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
            </button>
            ...
            <form (ngSubmit)="saveQuizQuestion()">
              <div class="form-group" style="margin-top: 1rem;">
                ...
                <textarea class="form-control" [(ngModel)]="quizForm.questionText" name="qText" rows="2" placeholder="e.g. Under US GAAP, how is depreciation expense classified?" required></textarea>
              </div>
              ...
              <div class="form-group">
                <label class="form-label">Explanation (Shown after student submits)</label>
                <textarea class="form-control" [(ngModel)]="quizForm.explanation" name="qExp" rows="2" placeholder="Explain the rationale..."></textarea>
              </div>

              <div class="modal-footer" style="display: flex; justify-content: flex-end; gap: 0.5rem; margin-top: 1rem;">
                <button type="button" (click)="showQuizModal.set(false)" class="btn btn-outline btn-sm">Cancel</button>
  ...
  styles: [`
    ...
    .emerald-icon { background: var(--emerald-light); color: var(--emerald); border: 1px solid var(--emerald-border); }
    .slate-icon { background: var(--bg-subtle); color: var(--primary); border: 1px solid var(--border-subtle); }
    .amber-icon { background: var(--amber-light); color: var(--amber); border: 1px solid var(--amber-border); }
    .cyan-icon { background: var(--cyan-light); color: var(--cyan); border: 1px solid var(--cyan-border); }
